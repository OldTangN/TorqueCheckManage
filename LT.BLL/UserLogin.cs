...
namespace LT.BLL
{
    public class UserLogin
    {
        ...
        role getrole()
        {
            ...
            try
            {
                ...
                _userinfo.user = u;
                List<LT.Model.UserToRoleModel.usertorole> r = UserToRole.selectbyuserid(_userinfo.user.guid);
                if (r == null || r.Count <= 0)
                    return ro = null;
    ...
    public class GetUser
    {
        ...
        public userinfo getuserinfo(List<users> u)
        {
            ...
            List<LT.Model.UserToRoleModel.usertorole> ur = new List<UserToRoleModel.usertorole>();
            ...
            if (s != null)
            {
                foreach (LT.Model.UserToRoleModel.usertorole um in ur)
                {
        ...
        public List<LT.Model.UserToRoleModel.usertorole> GetUserToRole(string systemname)
        {
            ...
            if (ur == null || ur.Count < 0)
                return null;
            List<LT.Model.UserToRoleModel.usertorole> utr = new List<UserToRoleModel.usertorole>();
            foreach (role r in ur)
            {
                List<LT.Model.UserToRoleModel.usertorole> temputr = UserToRole.selectbyroleid(r.guid);
                if (temputr != null)
                {
                    foreach (LT.Model.UserToRoleModel.usertorole role in temputr)
                    {
                        utr.Add(role);
                    }
                }
        ...
        public List<UserModel> GetUserModel()
        {
            ...
            try
            {
                ...
                foreach (LT.Model.UserToRoleModel.usertorole ur in utr)
                {
                    count++;
